...
let ClientService = class ClientService {
    ...
    getClient() {
        switch (this.network) {
            ...
            case 'mainnet':
                ...
                break;
            case 'custom':
                this.client = sdk_1.Client.forNetwork(this.custom.node).setMirrorNetwork(this.custom.mirror);
                break;
        }
    ...
    generateCustomClient(accountId, privateKey, environment) {
        console.log(`generating client for ${environment}, ${accountId}, ${privateKey}`);
        let client = null;
        switch (environment) {
            case 'testnet':
